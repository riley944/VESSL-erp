...
//
// vessl.programs still permits 'inquiry' in its CHECK. Narrowing the constraint
// is a later script; 0 rows carry it, so nothing is stranded meanwhile. See
// PLM.md.
...
//
// Sampling outranks Quoted, so a sampled-and-quoted program reads Sampling and
// its Quoted date is still visible on the card.
//
// Null for a program with nothing yet -- which, with Inquiry gone, means a
// program created by an order rather than a quote. It has no column on the board
// and the board does not pretend otherwise.
...
export function stageEnteredAt(stage, events, declaredStageAt) {
  ...
  if (stage === 'sampling') return declaredStageAt || null;
  return null;
}
