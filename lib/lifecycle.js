...
// A PLAIN DATE IS A CALENDAR DAY, NOT AN INSTANT. new Date('2026-07-21') is
// midnight UTC, which is the evening of Jul 20 anywhere west of Greenwich -- so
// every date column this formats (quote_date, order_date on POs and SOs,
// test_date) rendered one day early in US time. Built from its parts instead,
// the day stays the day it was entered as.
//
// Timestamps are untouched. issued_at and created_at are real instants, and
// showing them in the reader's own time zone is correct.
const PLAIN_DATE = /^(\d{4})-(\d{2})-(\d{2})$/;
export const fmt = d => {
  if (!d) return null;
  try {
    const m = typeof d === 'string' ? PLAIN_DATE.exec(d) : null;
...
//
// Script 54 narrowed the CHECK to NULL or 'sampling'. declared_stage and
...
// THE SAMPLING TEST, EXPORTED SO THERE IS ONE OF IT. Production implies sampling
// happened, which is the whole subtlety here, and it now has two readers -- the
// derived stage below, and the Sampling line in what the system knows on a PLM
// card. Two copies that agree today are two copies that disagree later.
//
// Returns the stage word itself rather than a boolean, because the caller that
// renders it needs to say WHICH of the two it found, and a boolean would send it
// back to reading product_stage on its own.
export const sampledFromStage = product => {
  const s = ((product || {}).product_stage || '').toLowerCase();
  return (s === 'sample' || s === 'production') ? s : null;
};

export function currentStage(events, product) {
  const p = product || {};
  if (events.tested || p.compliance_status === 'passed') return 'tested';
  if (sampledFromStage(p)) return 'sampling';
...
// Same trap as fmt. A plain quote or test date parsed with new Date() is
// midnight UTC, so from 8pm Eastern the card read one day longer in stage than
// it had been. A plain date is counted as whole calendar days between that day
// and today, both at local midnight -- rounded rather than floored, so a
// daylight-saving day of 23 or 25 hours still counts as one.
//
// Timestamps keep the elapsed-time count they always had.
export const daysSince = iso => {
  if (!iso) return null;
  const m = typeof iso === 'string' ? PLAIN_DATE.exec(iso) : null;
  if (m) {
