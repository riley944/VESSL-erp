...
//
// THE PIPELINE STOPS BEFORE BOTH OF THEM. Inquiry, Quoted, Sampling, Tested is
// unambiguous -- you cannot sample what was never inquired about, and testing
// follows a sample. The ambiguity that killed the first attempt now sits
// entirely on the far side of "complete", where this ladder never reaches.
//
...
export const PIPELINE_STAGES = [
  ['inquiry',  'Inquiry',  'declared'],
  ['quoted',   'Quoted',   'derived'],
  ['sampling', 'Sampling', 'declared'],
  ['tested',   'Tested',   'derived'],
];
...
//
// declared_stage HOLDS ONE VALUE, so a program declared Sampling carries no
// Inquiry record and never will. Furthest-reached resolves that -- Sampling
// outranks Quoted, so a sampled-and-quoted program reads Sampling and its Quoted
// date is still visible on the card. The alternative was two boolean columns to
// say something nobody has asked to know.
export function currentStage(events, declaredStage) {
  if (events.tested) return 'tested';
  if (declaredStage === 'sampling') return 'sampling';
  if (events.quoted) return 'quoted';
  if (declaredStage === 'inquiry') return 'inquiry';
  return null;
}
...
export function stageEnteredAt(stage, events, declaredStageAt) {
  if (stage === 'tested' || stage === 'quoted') return (events[stage] || {}).on || null;
  if (stage === 'sampling' || stage === 'inquiry') return declaredStageAt || null;
  return null;
}
