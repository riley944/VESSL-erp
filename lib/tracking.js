...
// trackingUrl(carrier, number, carriers) returns the carrier's own tracking page
// for the number, or null when it cannot tell whose number it is -- and null
// means the number is shown as plain text, never as a guess.
//
// THE LIST COMES FIRST. carriers is the companies of type carrier, each with
// companies.tracking_url (script 85), loaded with the board. A carrier whose
// name is on the list -- matched ignoring case and outer spaces, the rule the
// New Company form uses to adopt a carrier -- links through that row's
// pattern, with {number} replaced; a listed carrier with no pattern is plain
// text. Only a name NOT on the list falls through to the built-in four below,
// so the links keep working if the list fails to load.
//
...
const fromPattern = (row, n) =>
  row && row.tracking_url ? row.tracking_url.split('{number}').join(encodeURIComponent(n)) : null;

// The list row a carrier name refers to, or null.
export function findCarrier(carriers, name) {
  const k = norm(name);
  return k ? (carriers || []).find(c => norm(c.name) === k) || null : null;
}

export function trackingUrl(carrier, number, carriers = []) {
  const n = String(number || '').replace(/[\s-]+/g, '');
  if (!n) return null;
  const listed = findCarrier(carriers, carrier);
  if (listed) return fromPattern(listed, n);
  let key = carrierKey(carrier);
  if (key === 'other') return null;
  if (!key) {
    // A blank carrier, guessed from the number. The list row of that name
    // wins over the built-in pattern when there is one.
    key = inferFromNumber(n);
    if (!key) return null;
    const byName = { fedex: 'FedEx', ups: 'UPS', usps: 'USPS', dhl: 'DHL' }[key];
    const row = findCarrier(carriers, byName);
    if (row) return fromPattern(row, n);
  }
  return URLS[key](n);
}
