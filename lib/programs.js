...
// PLM is kept by hand since script 60. A card exists because somebody ticked
// Create PLM program on a quote, and it opens at the stage that tick means --
// quoted -- owned by the person who made the quote.
//
...
// owner and its notes. Ticking the box on a second quote for the same product and
// client must not drag a card back to quoted or take it off its owner.
// AN ID WINS OVER AN EMAIL, and skips the lookup entirely. Both doors ask who
// should own the card now and hand back the staff_profiles id the picker chose,
...
// it is stamped on the insert so a new card never reads as never touched. Both
// doors pass it -- the Create PLM Card popup and the tick on the quote form.
//
...
// had. Ticking the box on a second quote for the same pair cannot restamp it.
