...
export async function createProgram(productId, clientId, { stage = 'quoted', ownerEmail = null } = {}) {
  ...
  if (ownerEmail) {
    ...
    ownerId = (data && data[0] && data[0].id) || null;
  }
  const { error } = await SB.from('programs')
    .upsert([{ product_id: productId, client_company_id: clientId, declared_stage: stage, owner_id: ownerId }],
            { onConflict: 'product_id,client_company_id', ignoreDuplicates: true });
  return { created: !error, error: error || null, ownerId };
}
