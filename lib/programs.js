...
export async function createProgram(productId, clientId, { stage = 'quoted', ownerEmail = null, ownerId = null, createdBy = null } = {}) {
  ...
  const { error } = await SB.from('programs')
    // created_by is written HERE AND NOWHERE ELSE (script 78). It is the one
    // column that remembers who made the card; updated_by records the same
    // person now but is overwritten by the first edit.
...
// sets the product's own stage to production (syncProductStage).
//
...
// ── THE PRODUCT'S STAGE MIRRORS ITS CARD ────────────────────────────────────
// products.product_stage says what a PRODUCT is -- sample, production or not
// set -- and it follows the card on every move, forward or back, on decision:
//
//   production, shipped            -> 'production'
//   sampling, revision, testing    -> 'sample'
//   quoted                         -> NULL, Not set
//
// It REPLACES the forward-only rule, which never wrote 'sample' over
// 'production' and left Quoting alone. That rule protected a production product
// from a second client's sampling card and from a card dragged back by mistake;
// the decision is that the product should say where its card is, so a move back
// is a move back for the product too.
//
// LAST WRITER WINS. A product with two cards takes the stage of whichever moved
// last, and the Testing page select is overwritten by the next move of any card
// for that product. Creating a card is not a move -- createProgram does not call
// this -- so a new card at Quoting does not clear a product that is already set.
//
// ONLY A CHANGE IS WRITTEN. The filter skips a row already holding the target,
// so a move that changes nothing on the product writes nothing to it. Returns
// { changed, error } and never throws -- the card has already moved, and a
// product stage that did not follow is fixable by hand, and the caller toasts.
export async function syncProductStage(productId, cardStage) {
  if (!productId) return { changed: false, error: null };
  try {
    const target = (cardStage === 'production' || cardStage === 'shipped') ? 'production'
                 : (cardStage === 'sampling' || cardStage === 'revision' || cardStage === 'testing') ? 'sample'
                 : cardStage === 'quoted' ? null
                 : undefined;
    // A card with no stage, or a value this map does not know, says nothing
    // about the product, so nothing is written.
    if (target === undefined) return { changed: false, error: null };
    let q = SB.from('products').update({ product_stage: target }).eq('id', productId);
    q = target === null
      ? q.not('product_stage', 'is', null)
      : q.or('product_stage.is.null,product_stage.neq.' + target);
    ...
    for (const c of cards || []) {
      ...
      try {
        await SB.from('program_notes').insert({
          // Stamped Production -- the stage it announces -- so it shows while the
          // card is in Production (script 79, Card Notes are per stage).
          program_id: c.id, author: by || null, source: 'po-auto', stage: 'production',
