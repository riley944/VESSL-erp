...
//
// Conventions follow app/testing.jsx -- same card, Overlay, inp/lbl, and the
// global .prod-search class -- with one deliberate divergence: errors and
// confirmations go through window._toast rather than alert(). ToastProvider in
// page.jsx publishes that global and wraps this page, it has a real 'err'
// variant styled red in globals.css, and it does not block the modal. Please
// don't "correct" it back to alert() for consistency with testing.jsx.
...
// ── CREATE / EDIT ────────────────────────────────────────────────────────────
// Branches on data?.id. No delete: retiring a code is unchecking Active, so a
// quote that already cites it still resolves.
function CodeModal({ data, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    const { error } = editing
      ? await SB.from('htscodes').update(payload).eq('id', data.id)
      : await SB.from('htscodes').insert(payload);
    setSaving(false);
    if (error) {
