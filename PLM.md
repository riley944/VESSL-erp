...
> | **Removed, or deleted** | Two ways off the board, at the foot of the card's Sampling tab. **Remove from board** is the everyday one and is reversible: it sets `archived`, keeps the card, its notes and tasks, and files it in a *Removed* column behind *Show N removed*, from which *Put back on the board* returns it to its stage. **Delete card** — a quiet red link beside it — is for good: after a confirm naming the SKU, product and client and a typed `DELETE`, the `programs` row is deleted and its notes and checklist go with it by cascade. The product and its sampling log are untouched. Script 73 granted the DELETE; `anon` holds none. |
